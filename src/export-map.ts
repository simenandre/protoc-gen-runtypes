...
export class ExportMap {
  ...
  exportNested(
    ...
    const messageName =
      message.getName() ||
      throwError(`Missing message name for message. Scope: ${scope}`);
    ...
    if (messageOptions && messageOptions.getMapEntry()) {
      const keyType =
        ...
        throwError(
          `Missing map key type for message. Scope: ${scope} Message: ${messageName}`,
        );
      const keyTypeName = message.getFieldList()[0].getTypeName();
      const valueType =
        message.getFieldList()[1].getType() ||
        throwError(
          `Missing map value type for message. Scope: ${scope} Message: ${messageName}`,
        );
      const valueTypeName = message.getFieldList()[1].getTypeName();
      mapFieldOptions = {
        key: [keyType, keyTypeName ? keyTypeName.slice(1) : null],
        value: [valueType, valueTypeName ? valueTypeName.slice(1) : null],
      };
    }
    ...
    const fileName =
      ...
      throwError(
        `Missing file name for message. Scope: ${scope} Message: ${messageName}`,
      );
    ...
    fileDescriptor.getEnumTypeList().forEach((enumType) => {
      ...
      this.enumMap[packagePrefix + enumName] = {
        ...
        fileName:
          fileDescriptor.getName() ||
          throwError(
            `Missing file descriptor name for enum. Scope: ${scope} Enum: ${enumName}`,
          ),
        enumOptions: enumType.getOptions(),
      };
    });

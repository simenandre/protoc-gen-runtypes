...
import { ExportEnumEntry, ExportMessageEntry } from './export-map';
...
export function throwError(
  message: string,
  includeUnexpectedBehaviourMessage = true,
): never {
  if (includeUnexpectedBehaviourMessage) {
    throw new Error(
      `${message} - This is unhandled behaviour and should be reported as an issue on https://github.com/cobraz/protoc-gen-runtypes/issues`,
    );
  } else {
    throw new Error(message);
  }
...
export function isProto2(fileDescriptor: FileDescriptorProto): boolean {
  ...
  return (
    ...
    fileDescriptor.getSyntax() === PROTO2_SYNTAX
  );
}

export function oneOfName(name: string): string {
...
export function getPathToRoot(fileName: string): string {
